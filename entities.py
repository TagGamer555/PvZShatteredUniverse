    ...
    def draw(self, surface):
        if not self.collected:
            if self.health / self.max_health > 0.1:
                pg.draw.circle(surface["FG"], (255, 255, 0, min(255, self.existed_for*8)), (self.x, self.y), self.value/4)
            else:
                pg.draw.circle(surface["FG"], (255, 255, 0, max(0, min(255, self.health*8))), (self.x, self.y), self.value/4)
        else:
            pg.draw.circle(surface["FG"], (255, 255, 0, 255), (self.x, self.y), self.value/4)
    
    def die(self):
        if self.collected:
            self.dist_to_sun_ui = utils.dist(20, 20, self.x, self.y)
            if self.dist_to_sun_ui > 1: #TODO: Un-hardcode coords in the future
                self.x, self.y = utils.translate_point_degr(self.x, self.y, min(15, self.dist_to_sun_ui*0.1), utils.point2point_angle_degr(self.x, self.y, 20, 20))
            else:
...
class Plant(Entity):
    ...
    # I'm lazy and most plants will have this hitbox anyway
    def get_hitbox(self):
        size = 50
...
class Pea(Entity):
    ...
    def draw(self, surface):
        pg.draw.circle(surface["FG"], (127, 255, 0), (self.x, self.y), 10)  # Draw the pea as a circle
...
class Star(Entity):
    ...
    def draw(self, surface):
        pg.draw.circle(surface["FG"], (255, 255, 75), (self.x, self.y), 10)
...
class Peashooter(Plant):
    ...
    def draw(self, surface):
        pg.draw.rect(surface["MG"], (127, 255, 0), (self.x-25, self.y-25, 50, 50))  # Just a placeholder visual
...
class Sunflower(Plant):
    ...
    def __init__(self, x, y):
        super().__init__(x, y, 300)
        self.reload_reset = 1200 # nerf from 20s to 25s - too fast, not sorry; nvm, I'm reveting
    ...
    def draw(self, surface):
        ...
        if self.reload / self.reload_reset < 0.1:
            pg.draw.rect(surface["MG"], (255, 255, 120), (self.x-25, self.y-25, 50, 50))
        else:
            pg.draw.rect(surface["MG"], (255, 190, 0), (self.x-25, self.y-25, 50, 50))
...
class WallNut(Plant):
    ...
    def draw(self, surface):
        ...
        if self.health_left_percent > 0.6666667:
            pg.draw.rect(surface["MG"], (142, 82, 82), (self.x-25, self.y-25, 50, 50))
        # >1/3 health left. A good amount hurt
        elif self.health_left_percent > 0.3333333:
            pg.draw.rect(surface["MG"], (91, 49, 56), (self.x-25, self.y-25, 50, 50))
        # >0 health left. "Oh crap we're dying"
        else:
            pg.draw.rect(surface["MG"], (66, 36, 51), (self.x-25, self.y-25, 50, 50))
...
class Pumpkin(Plant):
    ...
    def draw(self, surface):
        ...
        # >2/3 health left. We're okay
        if self.health_left_percent > 0.6666667:
            pg.draw.rect(surface["MG+"], (249, 163, 27), (self.x-37.5, self.y, 75, 35))
        # >1/3 health left. A good amount hurt
        elif self.health_left_percent > 0.3333333:
            pg.draw.rect(surface["MG+"], (250, 106, 10), (self.x-37.5, self.y, 75, 35))
        # >0 health left. "Oh crap we're dying"
        else:
            pg.draw.rect(surface["MG+"], (223, 62, 35), (self.x-37.5, self.y, 75, 35))
...
class CherryBomb(Plant):
    ...
    def draw(self, surface):
        ...
        pg.draw.rect(surface["MG+"], (init_color[0]+red_progress, init_color[1]+green_progress, init_color[2]+blue_progress), (self.x-offset, self.y-offset, size, size))
...
class TerraFern(Plant):
    ...
    def draw(self, surface):
        pg.draw.rect(surface["BG"], (26, 122, 62), (self.x-30, self.y-12.5, 60, 40))
...
class Starfruit(Plant):
    ...
    def draw(self, surface):
        pg.draw.rect(surface["MG"], (255, 255, 75), (self.x-25, self.y-25, 50, 50))
...
class BasicZombie(Zombie):
    ...
    def draw(self, surface):
        pg.draw.rect(surface["MG++"], (150, 100, 200), (self.x-15, self.y-30, 30, 60))
...
class ConeheadZombie(Zombie):
    ...
    def draw(self, surface):
        ...
        if self.armor_durability > 0:
            ...
            pg.draw.rect(surface["MG++"], (255, 175, 75), (self.x-15, self.y-40, 30, 20))
...
class BucketheadZombie(Zombie):
    ...
    def draw(self, surface):
        pg.draw.rect(surface["MG++"], (150, 100, 200), (self.x-15, self.y-30, 30, 60))
        if self.armor_durability > 0:
            pg.draw.rect(surface["MG++"], (175, 175, 175), (self.x-15, self.y-40, 30, 20))
